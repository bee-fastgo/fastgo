...
/**
 * handler thread
 * @author luke 2016-1-16 19:52:47
 */
				...
				if (triggerParam!=null) {
					...
					if (idleTimes > 30) {
						if(triggerQueue.size() == 0) {	// avoid concurrent trigger causes jobId-lost
							SimpleJobExecutor.removeJobThread(jobId, "excutor idel times over limit.");
						}
					}
				}
			} catch (Throwable e) {
				if (toStop) {
					SimpleJobLogger.log("<br>----------- JobThread toStop, stopReason:" + stopReason);
				}
